...
CallerSensitive *CallerSensitive::callerSen = nullptr;

cJSON *CallerSensitive::parseCallerJson(std::string path)
{
    FILE *file = NULL;
...
// CallerSensitive::JavaDataType CallerSensitive::getBasicType(std::string type)
// {
...
// Type *CallerSensitive::getType(std::string type)
// {
...
// std::vector<Type *> CallerSensitive::getParams(std::vector<std::string> args)
// {
...
// void CallerSensitive::output2file(std::string outPath)
// {
//     std::error_code EC;
...
// Function *CallerSensitive::functionDeclarationIR(std::string funName, std::string ret, std::vector<std::string> args)
// {
...
// std::map<std::string, Value *> CallerSensitive::allocaStore(std::vector<std::string> params, Function *func)
// {
//     std::string arg = "arg";
...
// Value *CallerSensitive::allocaStoreLoad(std::string typeStr, Value *value)
// {
...
// Value *CallerSensitive::getCalleeArgs(std::string typeStr, std::string name, std::map<std::string, Value *> callerArgs, std::map<std::string, Value *> rets)
// {
//     Type *type = getType(typeStr);
...
// u32_t CallerSensitive::getNumber(std::string s)
// {
...
std::string CallerSensitive::getJsonFile(int &argc, char **argv)
{
...
// void CallerSensitive::callerIRCreate(std::string jsonPath, std::string irPath)
// {
...
void CallerSensitive::getCallees(std::string jsonPath)
{
        ...
        if (item != NULL)
        {
            ...
            while (obj)
            {
                if (obj && strstr(obj->string, "callee"))
                {
                    std::string funName;
                    // cJSON *callee = obj->child;
                    cJSON *callee = obj->child->next;
                    // if (strcmp(callee->string, "name") == 0)
                    if (strcmp(callee->string, "nativeName") == 0)
                    {
                        funName = callee->valuestring;
                        CallerSensitive::callees.push_back(callee->valuestring);
                    }
                    while(callee)
                    {
                        if (strcmp(callee->string, "aliasedArgs") == 0)
                        {
                            cJSON *arg = callee->child;
                            std::vector<int> args;
                            while(arg)
                            {
                            ...
                            CallerSensitive::tainedArgs[funName] = args;
                        }
                        callee = callee->next;
                    }
                }
    ...
    for (u32_t j = 2; j < fun.arg_size(); ++j)
    {
        ...
        for (auto eit = argNode->getCopyInEdges().begin(); eit != argNode->getCopyInEdges().end(); eit++)
        {
            ...
            for (auto it = srcNode->getAddrInEdges().begin(); it != srcNode->getAddrInEdges().end(); it++)
            {
                ...
                {
                    cm->isRetTained = true;
                    taintedCArgs.push_back(j-2);
                }
            }
        }
    }
    if (taintedCArgs.size() > 0)
    {
        cm->taintedArgs = taintedCArgs;
    }
}
...
void CallerSensitive::JavaFieldsAsCallBackArgs(std::vector<JavaField *> javaFields, CallSite callee, AndersenBase *ander, JavaMethod *jm)
{
    for (u32_t i = 2; i < callee.arg_size(); ++i)
    {
        ...
        for (std::vector<JavaField *>::iterator it = javaFields.begin(); it != javaFields.end(); ++it)
        {
            ...
            {
                jm->taintedFields[i - 3] = (*it)->fieldName;
            }
        }
    }
}
...
void CallerSensitive::CreateSpecification()
{
    ...
    for (std::vector<CMethod>::iterator it = cmethods.begin(); it != cmethods.end(); it++)
    {
        ...
        for (std::vector<JavaMethod *>::iterator it2 = it->javaMethods.begin(); it2 != it->javaMethods.end(); it2++)
        {
            ...
            {
                u32_t index = 1;
                for (std::map<int, std::vector<std::string>>::iterator it3 = (*it2)->taintedArgs.begin(); it3 != (*it2)->taintedArgs.end(); it3++)
                {
                    ...
                    cJSON *tainedJavaArgs = cJSON_CreateArray();
                    for (std::vector<std::string>::iterator it4 = it3->second.begin(); it4 != it3->second.end(); it4++)
                    {
                        cJSON_AddItemToArray(tainedJavaArgs, cJSON_CreateString((*it4).c_str()));
                    }
                    cJSON_AddItemToObject(tainedArgs, "tainedArg", tainedJavaArgs);
                    cJSON_AddItemToObject(JMethod, ("tainedArgs" + std::to_string(index++)).c_str(), tainedArgs);
                }
            }

            if ((*it2)->taintedFields.size() > 0)
            {
                u32_t index = 1;
                for (std::map<int, std::string>::iterator it3 = (*it2)->taintedFields.begin(); it3 != (*it2)->taintedFields.end(); it3++)
                {
                    cJSON *tainedArgs = cJSON_CreateObject();
                    cJSON_AddItemToObject(tainedArgs, "JavaArg", cJSON_CreateString(std::to_string((it3)->first).c_str()));
                    cJSON_AddItemToObject(tainedArgs, "tainedField", cJSON_CreateString((it3)->second.c_str()));
                    cJSON_AddItemToObject(JMethod, ("tainedArgs" + std::to_string(index++)).c_str(), tainedArgs);
                }
            }
        ...
        if (it->TaintedjavaMethod != "")
            cJSON_AddItemToObject(CMethod, "TaintedjavaMethod", cJSON_CreateString(it->TaintedjavaMethod.c_str()));
        if (it->taintedArgs.size() > 0)
        {
            /*
            cJSON *tainedCArgs = cJSON_CreateArray();
            for (std::vector<int>::iterator it3 = it->taintedArgs.begin(); it3 != it->taintedArgs.end(); it3++)
            {
                cJSON_AddItemToArray(tainedCArgs, cJSON_CreateString(std::to_string(*it3).c_str()));
            }
            cJSON_AddItemToObject(CMethod, "tainedArg", tainedCArgs);
            */

            for (std::vector<int>::iterator it3 = it->taintedArgs.begin(); it3 != it->taintedArgs.end(); it3++)
            {
                std::string arg = "arg" + std::to_string(*it3);
                cJSON_AddItemToObject(CMethod, "Ret", cJSON_CreateString(arg.c_str()));
            }
        }
